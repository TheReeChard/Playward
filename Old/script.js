...
function getCompletedSteps() {
  ...
  if (
    ...
    appData.diversity ||
    appData.medicalConsiderations.length > 0 ||
    Object.keys(appData.customization).length > 0
  ) {
    completed.push(2);
  }
...
function saveCustomTheme() {
  ...
  appData.themeCharacter = document.getElementById(
...
function loadCustomizationPage() {
  ...
  switch (appData.activityType) {
    ...
    case "trivia":
      ...
      subtitle.textContent =
        "Select the aspects of the trivia game you want to adjust here. These selections are optional.";
      html = `
...
function generatePrompt() {
  ...
  const replacements = {
    ...
    "{THEME}": themeToUse,
    "{CHARACTER_INFO}":
      appData.themeCharacter || "A kid superhero flying over the city",
    ...
    "{OBJECTS_TO_INCLUDE}":
      appData.customization["objects-include"] || "Various themed objects",
    "{ACTIVITY_LOCATION}":
      appData.customization["activity-location"] || "Bedroom",
